...
const TransactionList = ({ transactions, wallet }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setListHeight(newHeight > 0 ? newHeight : 200);
    };
    handleResize();
  ...
  const Row = ({ index, style }) => {
    ...
    const formatedDate = new Date(transaction.timeStamp * 1000)
      .toISOString()
      .split('.')[0]
      .replace('T', ' ');
    ...
    return (
      ...
      >
        ...
        <span
          className={`transaction-cell transaction-amount ${received ? 'transaction-amount-positive' : 'transaction-amount-negative'}`}
        >
          {received ? '+' : '-'}
          {roundedAmount} ETH
        </span>
